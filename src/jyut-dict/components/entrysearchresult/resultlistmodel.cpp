...
void ResultListModel::copyEntries(const std::vector<Entry> &entries, bool emptyQuery)
{
    ...
    disconnect(_updateModelTimer, nullptr, this, nullptr);

    // This if-statement exists to prevent in-between states where the list
    // suddenly contains zero entries.
    if (entries.empty() && !emptyQuery) {
        _updateModelTimer->setInterval(500);
        _updateModelTimer->setSingleShot(true);
        QObject::connect(_updateModelTimer,
                         &QTimer::timeout,
                         this,
                         [this, entries, emptyQuery] {
...
void ResultListModel::setWelcome()
{
    ...
    Entry entry{tr("Welcome!").toStdString(),
...
{
    if (!_isFavouritesList) {
        Entry entry{tr("No results...").toStdString(),
        ...
        Entry entry{tr("Nothing saved...").toStdString(),
