...
struct MandarinUtilsTests {
  ...
  @Test func segmentPinyinKeepGlobCharacters() async throws {
    #expect(
      segmentPinyin(text: "guang* dong?", removeGlobCharacters: false) == (
        true, ["guang", "* ", "dong", "?"]
      )
    )
  }
  ...
  @Test func segmentPinyinGlobCharactersTrimWhitespace() async throws {
    ...
    #expect(
      segmentPinyin(text: "guang?* ?????", removeGlobCharacters: false) == (
        true, ["guang", "?", "* ", "?", "?", "?", "?", "?"]
      )
    )
  ...
  @Test func segmentPinyinKeepSpecialCharacters() async throws {
    #expect(
      segmentPinyin(text: "guang？ dong1", removeSpecialCharacters: false) == (
        true, ["guang", "？", "dong1"]
      )
    )
  }
