...
Link = collections.namedtuple('Link', ['target', 'direct'])
        ...
        for index, line in enumerate(f):
            ...
            if lang == source:
                ...
                jyut = pinyin_jyutping_sentence.jyutping(trad,
                    tone_numbers=True, spaces=True)
                sentence_row = ChineseSentence(sentence_id=sentence_id,
                    trad=trad, simp=simp, pin=pin, jyut=jyut, lang=lang)
...
def parse_links_file(filename, sentences, nonchinese_sentences,
    chinese_sentences_filtered, nonchinese_sentences_filtered, intermediate_ids,
    links):
    ...
    with open(filename, 'r', encoding='utf8') as f:
        ...
        for line in f:
            ...
            if first_id in sentences and second_id in nonchinese_sentences:
                links[first_id] = Link(second_id, True)
        ...
        for key in first_intermediate:
            if key in second_intermediate:
                ...
                links[source_id] = Link(target_id, False)
...
if __name__ == '__main__':
    ...
    parse_links_file(sys.argv[3], chinese_sentences, nonchinese_sentences,
        chinese_sentences_filtered, nonchinese_sentences_filtered,
        intermediate_ids, links)
    write(chinese_sentences_filtered, nonchinese_sentences_filtered, links,
        sys.argv[1])
